...
// Pidfile includes role so multiple swarms (one per provider) can coexist
function defaultPidfile(role) {
  return path.join(process.env.HOME || '.', '.agentctl', `swarm-${role || 'default'}.pid`);
}
...
export class Supervisor extends EventEmitter {
  ...
  constructor(config = {}) {
    ...
    this.pidfile = config.pidfile || defaultPidfile(this.role);
